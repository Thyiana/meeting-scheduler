...
window.I18N = (function () {
  ...
  const dict = {
    zh: {
      ...
      'admin.data.import': '导入 Excel',
      'admin.data.importHint': '支持导出格式表头（日期/会议室/开始时间/结束时间/会议主题/主持人/联系电话备注），会议室按名称匹配，已存在的时间冲突会跳过并报告。',
      ...
      'admin.roomBanner.heading': '会议室专属公告',
      'admin.roomBanner.placeholder': '例如：本会议室临时改到 3 楼',
      'admin.roomBanner.hint': '只悬浮显示在该会议室的排期页面上，不影响其他会议室；与上面的全局公告可以同时显示。',
      'admin.backups.heading': '备份',
      'admin.backups.now': '立即备份当前排期',
      'admin.backups.hint': '每次执行"一键重置"前会自动生成一份快照，这里最多保留最近 30 份，随时可下载找回。',
      'admin.backups.empty': '暂无备份',
      'admin.backups.download': '下载',
      'admin.backups.created': '已生成备份（{count} 条预约）',
      ...
      'admin.reset.backupFailed': '重置已完成，但自动备份失败了——建议立即手动导出一份排期确认数据无误。',
      ...
      'admin.inviteModal.roomLabel': '生成链接的范围',
      'admin.inviteModal.roomAll': '全部会议室（嘉宾可自行切换）',
      'admin.inviteModal.roomHint': '选择某个会议室后，链接会锁定为只显示该会议室，嘉宾看不到其他会议室的排期——适合分开贴在每个会议室门口，保护各房间隐私。',
      'admin.meeting.suggestSlot': '💡 该会议室最近的空闲时段：{start} – {end}',
      'admin.meeting.suggestRooms': '💡 此时段其他空闲会议室：{rooms}',
      'admin.import.empty': '文件中没有可导入的数据行',
      'admin.import.done': '导入完成：成功 {imported} 条',
      'admin.import.partial': '导入完成：成功 {imported} 条，失败 {failed} 条（如："{reason}"）',
      'admin.import.parseFailed': '文件解析失败，请确认是有效的 .xlsx 文件',
      ...
      'guest.suggestSlot': '💡 该会议室最近的空闲时段：{start} – {end}',
      'guest.suggestRooms': '💡 此时段其他空闲会议室：{rooms}',
    ...
    en: {
      ...
      'admin.data.importHint': 'Uses the same header row as the export (Date/Room/Start/End/Topic/Host/Contact). Rooms are matched by name; conflicting rows are skipped and reported.',
      ...
      'admin.roomBanner.heading': 'Room-specific banner',
      'admin.roomBanner.placeholder': 'e.g. This room has temporarily moved to Floor 3',
      'admin.roomBanner.hint': 'Only shows on that room\'s own schedule page, never on other rooms; can be shown alongside the global banner above.',
      'admin.backups.heading': 'Backups',
      'admin.backups.now': 'Back up current schedule now',
      'admin.backups.hint': 'A snapshot is generated automatically every time "reset" runs; the most recent 30 are kept here and can be downloaded anytime.',
      'admin.backups.empty': 'No backups yet',
      'admin.backups.download': 'Download',
      'admin.backups.created': 'Backup created ({count} bookings)',
      ...
      'admin.reset.backupFailed': 'Reset completed, but the automatic backup failed — export the schedule manually right away to be safe.',
      ...
      'admin.inviteModal.roomLabel': 'Link scope',
      'admin.inviteModal.roomAll': 'All rooms (guest can switch freely)',
      'admin.inviteModal.roomHint': 'Pick a room and the link locks the guest page to just that room — guests never see other rooms\' schedules. Good for posting a separate link at each room\'s door for privacy.',
      'admin.meeting.suggestSlot': '💡 Nearest free slot in this room: {start} – {end}',
      'admin.meeting.suggestRooms': '💡 Other rooms free at this time: {rooms}',
      'admin.import.empty': 'No data rows found in the file',
      'admin.import.done': 'Import complete: {imported} succeeded',
      'admin.import.partial': 'Import complete: {imported} succeeded, {failed} failed (e.g. "{reason}")',
      'admin.import.parseFailed': 'Could not parse the file — please check it\'s a valid .xlsx',
      ...
      'guest.viewContact': 'Contact / Notes',
      'guest.suggestSlot': '💡 Nearest free slot in this room: {start} – {end}',
      'guest.suggestRooms': '💡 Other rooms free at this time: {rooms}',
      'guest.editBtn': 'Edit',
